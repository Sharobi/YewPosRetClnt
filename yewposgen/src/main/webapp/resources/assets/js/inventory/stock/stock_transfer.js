...
$(document).ready(function() {
	...
	$("#item_rate_ls").blur(function() {
		...
		var taxamt = 0;
		var gstamt = 0;
		//for dual stock calculation
		if (item_dual_stock==1) {
			var item_rate_ls = item_salerate_ls;
			var totlqty = parseFloat(pqty);// + parseFloat(Number(lqty));
		}else {
			var item_rate_ls = item_salerate_ls / conv;
			var totlqty = (pqty * conv) + Number(lqty);
		}
...
*/
	...
	$("#item_free").keyup(function() {
		...
		var item_taxPercentage = $("#item_taxPercentage").val();
		var itemmrppack = $("#item_mrp_pack").val();
		//var item_sale_rate =$("#item_sale_rate").val();
		var item_sale_rate =$("#item_rate_ls").val();
		var item_dis = $("#item_dis").val();
		var pqty = $("#item_pqty").val();
		var conv = $("#item_conv").val();
		var lqty = $("#item_lqty").val();
		...
		if (item_dual_stock==1) {
			  ...
			  pqty = parseFloat(pqty);// + parseFloat(Number(lqty));
		}else {
			 item_sale_rate = item_sale_rate / conv;
			 pqty = (pqty * conv) + Number(lqty);
		}


		total = parseFloat(pqty * item_sale_rate).toFixed(4);
		discamt = total * item_dis / 100;
		$("#item_discamt").val(parseFloat(discamt).toFixed(4));
		...
		{
			// LtAdj calculation start
			var totalret = parseFloat(pqty * item_sale_rate).toFixed(4);
			var modFree =  item_free % 1;
			var pqtyPerFree = pqty/modFree;
			ltadj = totalret/pqtyPerFree;
			$("#item_ltadj").val(parseFloat(ltadj).toFixed(4));
...
function calculateTotalamtAfterDiscount() { 
	...
	$('#selitem tbody tr').each(function() {
		var saleamt = $(this).find("#saletabamt").html();
		//if ($(this).find("#saletabisfreeitemaginstitem").html()==0) {// IAI ignor  when 0
			totsaleamt = totsaleamt + Number(saleamt);
		//}
...
/*function saveOrUpdateSaleInv() {
	...
	if ($("#item_id").val() == 0 || $("#item_id").val() == "") {
		...
		$('#selitem > tbody  > tr').each(function(i) {
			...
			saledetails.remarks=$(this).find('#saletabremarks').text(); //new added 7.5.2019
...
/*function paySaleInvModal() {
	...
	if (sale_id_present==0) {
		getvendorledger_sale($('#debitor_group_codef').val(),0,$('#salecustid').val(),3);// for sunndry debitor

		if($('#salecustid').val()!=0)
		{
		getvendorledger_sale($('#cash_codef').val(),0,0,5);// for cash
		}
 ...
 document.getElementById('alertmessagecont').innerHTML = "";
	...
	if ($("#item_id").val() == 0 || $("#item_id").val() == "") {
		...
		$('#selitem > tbody  > tr').each(function(i) {
			...
			var totlooseqty = (parseFloat($(this).find("#saletabpqty").text()) * parseFloat($(this).find("#saletabconv").text())) + parseFloat($(this).find("#saletablqty").text());
			saledetails.itemId = itemid.split("_")[0];
			var saleitembatch = $(this).find("#saletabbat").text();//new added 13.6.2019
			if(saleitembatch == ""){saleitembatch=0;}//new added 13.6.2019
			...
			saledetails.looseUnitId = $(this).find("#saletablunitid").text();
			saledetails.conversion = $(this).find("#saletabconv").text();
			saledetails.mrp = ($(this).find("#saletabmrppack").text()=="")?0:$(this).find("#saletabmrppack").text(); // changed 4.7.2019
			...
			saledetails.mrpPerUnit = $(this).find("#saletabmrpperunit").text();// parseFloat(parseFloat($(this).find("#saletabmrppack").text())/ parseFloat($(this).find("#saletabconv").text())).toFixed(2);
			saledetails.ratePerUnit = $(this).find("#saletabrateperunit").text();
			saledetails.rate = $(this).find("#saletabrateperunit").text() * parseFloat($(this).find("#saletabconv").text());  // $(this).find("#saletabitemsalerate").text(); // changed 4.7.2019
			saledetails.vat = 0;// $(this).find("#saletabvat").text();
			saledetails.vatPer = $(this).find("#saletabvatperc").text();
			saledetails.tax = 0;//$(this).find("#saletabtax").text();
			saledetails.taxPer = $(this).find("#saletabtaxperc").text();
			...
			saledetails.taxMode = $(this).find("#saletabitemtaxmode").text();
			saledetails.purchaseCostPerUnit = $(this).find("#saletabpurcostperunit").text();
			saledetails.saleRate = $(this).find("#saletabrateperunit").text() * parseFloat($(this).find("#saletabconv").text()); //$(this).find("#saletabitemsalerate").text();  // changed 4.7.2019
			saledetails.purchaseRate = $(this).find("#saletabitempurrate").text();
			saledetails.taxTypeId = $(this).find("#saletabitemtaxtype").text();
			saledetails.isExclusive = $(this).find("#saletabitemtaxtype").text();
			saledetails.retailTypeId=$("#retailtypeinfo").val();
...
function getClickeditmdet(clickitmdet) {
	...
	/*if(clickitmdet.locationRequired==null||clickitmdet.locationRequired==0){
		var seslocid=clickitmdet.locationId;
		//alert(seslocid);
...
//		$("#location").val(seslocid);
	...
	}else if(clickitmdet.locationRequired==1){
		var seslocid=clickitmdet.locationId;
		//alert(seslocid);
...
//		$("#location").val(seslocid);
	...
	if(clickitmdet.sizeWiseStockRequired==null||clickitmdet.sizeWiseStockRequired==0){
	...
	}else if(clickitmdet.sizeWiseStockRequired==1){
		...
		$("#Sizelabel_td").removeClass('hide');
		$("#size_td").removeClass('hide');
		$("#size > [value=" + clickitmdet.size + "]").removeAttr("selected");
		$("#size").prop('disabled', false);
	}
...
//		$("#colour").val(0);
		$("#colour > [value=" + clickitmdet.colour + "]").attr("selected", "true");
...
//		$("#colour").val(0);
		$("#colour > [value=" + clickitmdet.colour + "]").attr("selected", "true");
		$("#colour").prop('disabled', 'disabled');
	}else{
		$("#Colorlabel_td").removeClass('hide');
		$("#color_td").removeClass('hide');
//		$("#colour").val(0);
		$("#colour > [value=" + clickitmdet.colour + "]").removeAttr("selected");
		$("#colour").prop('disabled', false);
	}
...
function itemHeaderDivView(itemid) {
	...
	$("#current_stock").val($("#selitem tr#" + itemid).find('#saletabitemstkqty').text());
  ...
  if ($("#selitem tr#" + itemid).find('#saletabsalesmanIdComPer').text().split("_")[0]==0) {
	...
	$("#item_code").val($("#selitem tr#" + itemid).find('#saletabcode').text()); // new added 7.5.2019
	$("#itmremarks").val($("#selitem tr#" + itemid).find('#saletabremarks').text()); // new added 7.5.2019
...
function addOrUpdateItemToDetailsTable(operation) {
	...
	if($("#duedate").val()=='')
	{
		 $('#alertMsg').html('Please add due date ' );
		return false;
	}
	...
	var isfree_color=isfreeIAI==1? 'freeiai':'';
		...
		ajaxCallObject.callAjaxPost(BASE_URL + "/stock/getcurrstockbybatexpmrp.htm", CommResultsetObj, function(response) {
			...
			for ( var i = 0; i < itemcurstkdets.length; i++) {
				...
				//	console.log("response=" + itemcurstkdet.calculateLooseQty);
				var calculateLooseQty = itemcurstkdet.calculateLooseQty;
				curstockqty=calculateLooseQty;
				if ($("#item_pqty").val() > calculateLooseQty) {
				...
				else
				{
					...
					/*if (($('#saletabreceivepqty').val() == "") || ($('#saletabreceivepqty').val() == null)) {
						free = 0;
					} else {
						free = isfreeIAI==0?$('#saletabreceivepqty').val():0;
					}*/

					var purcost = (($('#item_pqty').val() * $('#item_conv').val()) + Number($('#item_lqty').val())) * $('#item_purCostperUnit').val();

					if (item_dual_stock==1) {
						//var calamt = (($('#item_pqty').val()) + Number($('#item_lqty').val())) * $('#item_rate_ls').val();
...
//					var looseQty = "<td id='saletablqty'>" + lqty + "</td>";
						...
						///freetd = "<td id='saletabfree' class='hide>" + free + "</td>";					
				...
				/*	var salems_amt="<td id='saletabsalemanAmt'>" + parseFloat(sales_mans_amt).toFixed(4) + "</td>";*/
				   /*
				    * for sale man amt
				    */
					...
					var itemcode = "<td id='saletabcode' class='hide'>" + $('#item_code').val() + "</td>"; // new added 7.5.2019
					var itemremarks = "<td id='saletabremarks' class='hide'>" + $('#itmremarks').val() + "</td>"; // new added 7.5.2019
					...
					if (operation == 1) { // edit
						...
						/*if (($('#item_free').val() == "") || ($('#item_free').val() == null)) {
							free = 0;
						} else {
...
//						$("#selitem tr#" + itemid).find('#saletablqty').text(lqty);
						/*$("#selitem tr#" + itemid).find('#saletabfree').text(free);*/
						...
						$("#selitem tr#" + itemid).find('#saletabcode').text($('#item_code').val()); // new added 7.5.2019
						$("#selitem tr#" + itemid).find('#saletabremarks').text($('#itmremarks').val()); // new added 7.5.2019
						
						
						
					} else {
//						createdrowline = starttrline + name + batchno + expiryDateFormat + manfacname + packQty + looseQty + conversion + mrppack + mrp + amt + rateperunit + vatperc + item_taxPercentage + vatamt + discperc + discamt + totamt + rowdelete + itembarcode + punitid + lunitid + content + stockedqty + mrpperunit + rate + schename + taxperc + taxamt + contentid + item_taxId + item_isGroupTax + item_discount + item_isDiscount + item_maxDiscountLimit + item_CalcTaxAmt + item_taxMode + item_hsnCode + item_purCost + item_purCostperUnit + salerate + item_purChase_rate+item_ltadj + endtrline;
...
function DoConfirmStockTransfer() {
	
	var trId=$("#confirmid").val();
	var type=$("#confirmtype").val();
	var detailsId=$("#confirmdetailsid").val();
	var itemId=$("#confirmitemid").val();
	var trQty=$("#confirmtrqty").val();
	...
	var ajaxCallObject = new CustomBrowserXMLObject();
	ajaxCallObject.callAjaxPost(BASE_URL + "/stock/deleteStockTransfer.htm", CommonRelsetmapperObj, function(response) {
		$('#pleasewaitModal').modal('hide');
		if (response == '1') {
			document.getElementById('confirmmessagecont').innerHTML = getStockTransInvText.dataSucDelete;
			showConfirmModal();
			
			
		} else {
			document.getElementById('confirmmessagecont').innerHTML = getStockTransInvText.dataNotDelete;
			showConfirmModal();
	...
	if(type==1){// for dispatch
		
		var CommonRelsetmapperObj = {};
		CommonRelsetmapperObj.transferId = trId;

		var ajaxCallObject = new CustomBrowserXMLObject();
		ajaxCallObject.callAjaxPost(BASE_URL + "/stock/dispatchStockTransfer.htm", CommonRelsetmapperObj, function(response) {
			$('#pleasewaitModal').modal('hide');
			if (response == '1') {
				document.getElementById('confirmmessagecont').innerHTML = getStockTransInvText.dataSucDispached;
				showConfirmModal();
				
				
			} else {
				document.getElementById('confirmmessagecont').innerHTML = getStockTransInvText.dataNotDispached;
				showConfirmModal();
	...
	if(type==2){// for restocking
		
		var  StockTransferDetailsObj = {};
		
		 StockTransferDetailsObj.stockTransId = trId;  
		 StockTransferDetailsObj.id = detailsId;  
		 StockTransferDetailsObj.itemId = itemId;  
		 StockTransferDetailsObj.transitPackQty = trQty;
		 
         console.log(StockTransferDetailsObj);
         
		var ajaxCallObject = new CustomBrowserXMLObject();
		ajaxCallObject.callAjaxPost(BASE_URL + "/stock/restocktransitqty.htm", StockTransferDetailsObj, function(response) {
			$('#pleasewaitModal').modal('hide');
			if (response == '1') {
				document.getElementById('confirmmessagecont').innerHTML = getStockTransInvText.datastkSucc;
				showConfirmModal();
				
				
			} else {
				document.getElementById('confirmmessagecont').innerHTML = getStockTransInvText.datastknotSucc;
				showConfirmModal();
...
if(type==3){// for close
	...
	var CommonRelsetmapperObj = {};
		...
		CommonRelsetmapperObj.transferId = trId;				          
		var ajaxCallObject = new CustomBrowserXMLObject();
		ajaxCallObject.callAjaxPost(BASE_URL + "/stock/closeStockTransferSend.htm", CommonRelsetmapperObj, function(response) {
			$('#pleasewaitModal').modal('hide');
			if (response == '1') {
...
function deleteStockTransfer(pid) {
...
function dispatchStockTransfer(pid) {
	$('#confirmModalStock').modal('show');
	$("#confirmid").val(pid);
	$("#confirmtype").val(1); // for post set as one		
}

function setTransitItemToResock(pid,id,itid,stkqty){
	$('#confirmModalStock').modal('show');
	$("#confirmid").val(pid);
	$("#confirmdetailsid").val(id);
	$("#confirmitemid").val(itid);
	$("#confirmtrqty").val(stkqty);
	$("#confirmtype").val(2);
			
}

function closeStockTransferSend(pid) {
	$('#confirmModalStock').modal('show');
	$("#confirmid").val(pid);
	$("#confirmtype").val(3); // for post set as one		
}
/*function targetURL(){
...
function targetURL() {
	...
	if (type == 0) {		
		...
		location.href = "#";
		  ...
		  location.href = BASE_URL + '/stock/stocktransferheader/'+res+'.htm';
	    }
  else if (type == 2) {
	  location.href = BASE_URL + '/stock/stocktransferheader/'+res+'.htm';
    }
...
/*function okRetAdjMod() {
	...
	//$("#cashAmt").val(parseFloat(Number(nettot) - Number(totretadjamt)).toFixed(2));
	//$("#creditAmt").val(parseFloat(Number(nettot) - Number(totamt)).toFixed(2));
}*/
...
//	alert("length="+$("input:checkbox[class=chk]:checked").length);
   ...
   /* alert("srate="+srate);
    alert("max="+Math.max.apply(Math, srate));
    var lqty = $("#item_lqty").val();
	var item_vat = $("#item_vat").val();
	var item_tax = $("#item_tax").val();
	var item_taxPercentage = $("#item_taxPercentage").val();
	...
	var item_rate_ls = $("#item_rate_ls").val();
	var conv = $("#item_conv").val();
	//var totlqty = (pqty * conv) + Number(lqty);length
	var totlqty = Number(length);
   ...
   // total = parseFloat(totlqty * item_rate_ls).toFixed(4); prev
	total = parseFloat(totlqty * maxslrate).toFixed(4);
...
//============================= after item search it will automatically calculate the total value
...
function onblur_cash_amt() {
	...
	if (cash_sale==0) { // when customer is present
		...
		getvendorledger_sale($('#cash_code1').val(),0,0,5);// for sunndry creditor
...
function getItemDetails_so(itemid) {
	...
	//	console.log("___current_stock___resp="+resp);
		if(itemdetail.length==1){// when no stock
			...
			createItemStockDetails_so(resp, itemid);// if batch present then it will open modal
...
function createOrUpdateStockTransfer(){
	...
	if($("#item_name").val())
		{ $('#alertMsg').html('Please Add or Update this Item ' );
			return false; }
	
	if($("#totitmcount").html()==0)
	{ $('#alertMsg').html('Please add item' );
		return false; }
	...
	StockTransferObj.stockTransDate=$("#date").val();
	StockTransferObj.sendGrossAmount=$("#totgrossamt").val();
	StockTransferObj.recvdGrossAmount=$("#totgrossamt").val();
	StockTransferObj.transitGrossAmount=0.0;
	StockTransferObj.sendDiscAmount=$("#totdiscamt").val();
	StockTransferObj.recvdDiscAmount=$("#totdiscamt").val();
	StockTransferObj.transitDiscAmount=0.0;
	StockTransferObj.sendTaxAmount=$("#totgstamt").val();
	StockTransferObj.recvdTaxAmount=$("#totgstamt").val();
	StockTransferObj.transitTaxAmount=0.0;
	StockTransferObj.sendNetAmount=$("#nettot").val();
	StockTransferObj.recvdNetAmount=$("#nettot").val();
	StockTransferObj.transitNetAmount=0.0;
	StockTransferObj.sendRoundoff=$("#roundoff").val();
	StockTransferObj.recvdRoundoff=$("#roundoff").val();
	StockTransferObj.transitRoundof=0.0;
	StockTransferObj.sendTotalMrp=$("#totmrpamt").val();
	StockTransferObj.recvdTotalMrp=$("#totmrpamt").val();
	StockTransferObj.transitTotalMrp=0.0;
	StockTransferObj.isDeleted=0;
	StockTransferObj.sendIsPosted=0;
	StockTransferObj.recvIsPosted=0;
	StockTransferObj.dueDate=$("#duedate").val();
	StockTransferObj.sendRemarks=$("#remarks").val();
	StockTransferObj.receiveStatus="N";
	StockTransferObj.dispatchStatus="N";
	StockTransferObj.receivedDate="Null";
	StockTransferObj.receivedBy=0;
	...
	$(tablerows).filter(function(){
		...
		StockTransferDetailsObj.batchNo=$(this).find("td#saletabbat").html();////$(this).find("#saletabexpdt").text();
		StockTransferDetailsObj.mfdDate=0;
		StockTransferDetailsObj.expiryDate=$(this).find("td#saletabexpdate").html();
		...
		StockTransferDetailsObj.sendPackQty=$(this).find("td#saletabpqty").html();
		StockTransferDetailsObj.recvdPackQty=$(this).find("td#saletabpqty").html();
		StockTransferDetailsObj.transitPackQty=0;
		...
		StockTransferDetailsObj.rate=$(this).find("td#saletabitemsalerate").html();
		...
		StockTransferDetailsObj.mrpPerUnit=$(this).find("td#saletabmrpperunit").html();///saletabpqty
		StockTransferDetailsObj.ratePerUnit=$(this).find("td#saletabrateperunit").html();
		StockTransferDetailsObj.sendItemGrossAmount=$(this).find("td#saletabamt").html();
		StockTransferDetailsObj.recvdItemGrossAmount=$(this).find("td#saletabamt").html();
		StockTransferDetailsObj.transitItemGrossAmount=0.0;
		StockTransferDetailsObj.discPer=$(this).find("td#saletabdiscperc").html();
		StockTransferDetailsObj.sendItemDiscAmount=$(this).find("td#saletabdiscount").html();
		StockTransferDetailsObj.recvdItemDiscAmount=$(this).find("td#saletabdiscount").html();
		StockTransferDetailsObj.transitItemDiscAmount=0.0;
		...
		StockTransferDetailsObj.sendItemTaxAmount=$(this).find("td#saletabitemcalcgstamt").html();
		StockTransferDetailsObj.recvdItemTaxAmount=$(this).find("td#saletabitemcalcgstamt").html();
		StockTransferDetailsObj.transitItemTaxAmount=0.0;
		StockTransferDetailsObj.sendItemNetAmount=$(this).find("td#saletabtotamt").html();
		StockTransferDetailsObj.recvdItemNetAmount=$(this).find("td#saletabtotamt").html();
		StockTransferDetailsObj.transitItemNetAmount=0.0;
		StockTransferDetailsObj.actSendPackQty=$(this).find("td#saletabpqty").html();
		StockTransferDetailsObj.actSendLooseQty=0;
		...
		StockTransferDetailsObj.createdDate=$("#date").val();
	...
	////console.log("StockTransferObj ",StockTransferObj);
		...
		setTimeout(function() {			
			var ajaxCallObject = new CustomBrowserXMLObject();
			ajaxCallObject.callAjaxPost(BASE_URL + "/stock/createOrUpdateStockTransfer.htm", StockTransferObj, function(response) {
				$('#pleasewaitModal').modal('hide');
				if (response == '0') {
					document.getElementById('confirmmessagecont').innerHTML = getStockTransInvText.dataNotAdd;
					$("#confirmval").val(0);
					showConfirmModal();
				} else {
					document.getElementById('confirmmessagecont').innerHTML = getStockTransInvText.dataSucAdd;
					$("#confirmval").val(response);
					showConfirmModal();
...
function getItemDetailsFromSo(itemdetails) {
	...
	//	console.log("___current_stock___resp="+resp);
		if(itemdetail.length==1){// when no stock
			...
			createItemStockDetails(resp, itemdetails.itemId);// if batch present then it will open modal
			$("#stkdetModal").modal("show");
		}
	}, null);
}
