...
function getvendordisval() {
	...
	if(!$("#purtype").is(":checked"))
	{
		/*if($('#seldistributor').val()==0)
		{
			return;
		}*/
		getpendingpurchasechallans();
	}
...
//,["addrs",addrs_field],["pin",pin_field],["phn1",phn1_field],["rgstratn",rgstratn_field],["licenceNo",licenceNo_field]
		...
		if($("#itemid").val()==0 || $("#itemid").val()=="")
		{
			...
			$("#peitem tr").not('thead tr').each(function(	i,v) {
				...
				if($(this).find('#tbl_mode').text()=="R")
				{
					...
					PurchaseRetDetailsObj.purbarcode = $(this).find('#tbl_purbarcode').text();
				...
				{ 
					...
					PurchaseDetailsObj.purbarcode = $(this).find('#tbl_purbarcode').text();
			...
			setTimeout(function()
			{
			   ...
			   if (error==0) {// error checking
				   $('#pleasewaitModal').modal('show');
					...
					if($("#purtype").is(":checked"))
					{
						PurchaseObj.isDirect = 1;
						crtupdpurdet = "/purinv/createorupdatepurchaseinvoicedirect.htm";
					}
					else
					{
						PurchaseObj.isDirect = 0;
						PurchaseObj.purchaseIDList = idList;
						crtupdpurdet = "/purinv/createorupdatepurchaseinvoiceindirect.htm";
						/*alert("Procedure and service pending");
						return;*/
					}
					ajaxCallObject.callAjaxPost(BASE_URL + crtupdpurdet, PurchaseObj, function(response) {
						$('#pleasewaitModal').modal('hide');
...
function targetURL(){
	...
	if(result==0){
		location.href = BASE_URL + '/purchaseinvoice/loadpurchaseinvoice.htm';
	}
	else if(result==-1){
		location.href = "#";
	}
	...
	else if($("#confirmval").val()=="itemAddErr")
	{
		...
		{
			location.href = BASE_URL + '/purinv/loadpurinvoicedet/'+result+'.htm';
		}
	}
}
...
function targetActionPur(){
	...
	if(result==0){
		...
		if ($('input[name=printPurchase]').is(":checked"))
		{
			...
			//location.href = BASE_URL + "/purinv/printPurInvoice.htm?backUrl=loadpurchaseinvoice&pid=" + result;
			/*location.href = BASE_URL + "/purchaseinvoice/printPurchaseInvoice.htm?backUrl=loadpurchaseinvoice&pid=" + result;*/
			location.href = BASE_URL + "/purchaseinvoice/printPurchaseInvoice.htm?backUrl=loadpurchaseinvoicedet&pid=" + result;/*Sayantan Mahanty added date-20/02/2020*/
			
			}
		else
		{
			location.href = BASE_URL + '/purchaseinvoice/loadpurchaseinvoicedet/'+result+'.htm';
		}
	}
}
		...
		ajaxCallObject.callAjaxPost(BASE_URL + "/purorder/getpurorderdetbyordernoforpi.htm", CommonRelsetmapperObj, function(response) {
			...
			if (purdetails.length == 0 || $("#billno").val()=="") {
				...
				for ( var i = 0; i < purdetails.length; i++) {
					...
					var barcode = "<td id='itmsku_'"+purdetail.itemId+">" + purdetail.sku + "</td>";
					...
					var endtrline = "</tr>";
					createdrowline = starttrline + chkbox + itmname + barcode + batch + exp + packQty + mrp + rate + taxper + discper + slnos + endtrline;
	...
	$('#searchordermodtable tbody tr').each(function() {
		...
		if (saleretvendorid == 0 || saleretvendorid == itemdetail.distributorId || itemdetail.distributorId==0) {
			...
			if (uniquechk == 1) {
				...
				var rowsku = '<td id="tbl_purbarcode">' + itemdetail.sku + '</td>';
				...
				//tr = tr + row1 + row2 + row3 + row4 + row5 + row6 + row7 + row8 + row9 + row10 + row11 + row12 + row13 + row14 + row15 + row16 + row17 + row18 + row19 + row20 + row21 + row22 + row23 + row24 + row25 + row26 + row27 + row28 + row29 + packunitid + isFreeRow + ltadjRow +'</tr>';
				tr = tr + row1 + rowsku + row2 + row3 + row4 + row7 + previousFree + row9 + tbl_sale_rate + row22 + row11+taxTypeId + row24 + row13 + row14 + netamt + row16 + row17 + row18 + row19 + row20 + taxid + taxmode + isgrptax + row25 + row26 + row27 + row28 + row29 + packunitid + isFreeRow + ltadjRow + sku + hsn + poId + predprbfrspdp + billpackqty + mode + returnReason+expmonth+mfd+location+ row5 + row6+ row8 + row15+slnos+slnoreq + item_batchWiseStock_req+item_expiryDateRequired_req+item_expiryMonthRequired_req+item_dualStockRequired_req+item_mrpRequired_req+item_locationRequired_req+item_priceListRequired_req+'</tr>';
				$("#peitem tbody").prepend(tr);
			}
		} else {
...
function fillItemRetDetailsDiv(purdetails)
{
	if (purdetails.length == 0) {
		...
		for ( var i = 0; i < purdetails.length; i++) {
			...
			var stockQty = "<td>" + purdetail.stockQty + "</td>"/*Sayantan Mahanty added date-26/02/2020*/
...
function insertModData(itemdetail) {
	...
	$("#pqty_ret").val(pqty);
	$("#stkqty_ret").val(itemdetail.stockQty);//*Sayantan Mahanty added date-26/02/2020*/
...
function addForRet() {
	//Sayantan Mahanty added date-25/02/2020
	var currentsqty=parseInt($("#stkqty_ret").val());
	var pkqty_r=$("#pqty_ret").val();
	if(pkqty_r>currentsqty){
		$("#crstk").text(currentsqty);
		$('#quantityIsnoAv').modal('show');
		return false;
	}
	...
	else if(itempresent==2)
	{
		...
		var sku = '<td id="tbl_sku">' + $("#barcode_ret").val() + '</td>';
		...
		var currentsqty_ret="<td class='hide' id='tbl_ret_currentsqty'>"+currentsqty+"</td>";
		// for common retail
		...
		tr = tr + row1 + sku + row2 + row3 + row4 + row7 + previousFree + row9 + tbl_sale_rate + row22 + row11 + row24 + row13 + row14 + netamt + row16 + row17 + row18 + row19 + row20 + taxid + taxmode + isgrptax + row25 + row26 + row27 + row28 + row29 + packunitid + isFreeRow + ltadjRow + hsn + poId + predprbfrspdp + billpackqty + mode + returnReason+expmonth+mfd+location+ row5 + row6+ row8 + row15+slnos+slnoreq + item_batchWiseStock_req+item_expiryDateRequired_req+item_expiryMonthRequired_req+item_dualStockRequired_req+item_mrpRequired_req+item_locationRequired_req+item_priceListRequired_req+chkdSlNos+currentsqty_ret+'</tr>';
		$("#peitem tbody").prepend(tr);
...
function itemDetailViewRet(trId,batchNo) {
	...
	$("#stkqty_ret").val($("#tblrow_R_" + trId+"_"+batchNo).find('#tbl_ret_currentsqty').text());//Sayantan Mahanty added date-26/02/2020
	...
	$("#stkqtyid_ret").val($("#tblrow_R_" + trId+"_"+batchNo).find('#tbl_ret_currentsqty').text());//Sayantan Mahanty added date-26/02/2020
	...
	$("#stkqty_ret").val($("#tblrow_R_" + trId+"_"+batchNo).find('#tbl_ret_stkqty').text());
...
function editForRet()
{
	...
	var currentsqty=parseInt($("#stkqtyid_ret").val());
	var pkqty_r=$("#pqty_ret").val();		
	if(pkqty_r>currentsqty){
	   $("#crstk").text(currentsqty);
	 $('#quantityIsnoAv').modal('show');
...
//For new purchase invoice changes

$("#purtype").click(function(e){
	console.log("seldistributor val = "+$("#seldistributor").val());
	if($("#seldistributor").val() == '0')
	{
		e.preventDefault();
		...
		$("#confirmval").val(-1);
		document.getElementById('confirmmessagecont').innerHTML = "Please select a vendor first.";//getPurInvText.vendorChangeError;
		showConfirmModal();

		return;
	}
	if($("#peitemtbody tr").length>0)
	{
		e.preventDefault();
		//alert("Please delete items before changing.");

		$("#confirmval").val(-1);
		document.getElementById('confirmmessagecont').innerHTML = "Please delete items before changing.";//getPurInvText.vendorChangeError;
		showConfirmModal();
	...
	if(!$(this).is(":checked"))
	{
		$("#purMode,#add_btn,#bill_label,#bill_no,#bill_date_label,#bill_date").addClass("hide");
		// $("#item_name,#purbarcode").attr("readonly",true);
	...
	{
		$("#purMode,#add_btn,#bill_label,#bill_no,#bill_date_label,#bill_date").removeClass("hide");
		// $("#item_name,#purbarcode").attr("readonly",false);

		$("#header_div").removeClass("hide");
		$("#pendingPurchaseChallanDiv").addClass("hide");
...
function getpendingpurchasechallans()
{
	var challanIds = parseInt($("#seldistributor").val());
	var ajaxCallObject = new CustomBrowserXMLObject();
	ajaxCallObject.callAjax(BASE_URL + "/purinv/getpendingpurchasechallans/" + challanIds + ".htm", function(resp) {
		var obj = jQuery.parseJSON(resp);
		console.log("Pending purchase challans : "+obj);
		creatependingPurchaseChallanDetails(obj);
	}, null);
}

function closependingPurchaseChallanDiv(){
	$("#pendingPurchaseChallanDiv").addClass("hide");
	//$("#purtype").click();
}

function creatependingPurchaseChallanDetails(pendingPurchaseChallans){
	$("#pendingPurchaseChallanDiv").removeClass("hide");
	$("#pendingPurchaseChallanBody").html("");
	//$("#itemnameofpurhis").text("");
	if($("#seldistributor").val()==0)
	{
		$("#pendingPurchaseChallanBody").html("<tr><td colspan='9'>Please Select a vendor to continue.</td></tr>");
	}
	else {
		for ( var i = 0; i < pendingPurchaseChallans.length; i++) {
			var pendPur = pendingPurchaseChallans[i];
			//console.log("invoice no = "+pendPur.invNo);
			var row = "<tr>"
				+ "<td><input name='purId' type='checkbox' id='"+pendPur.invNo+"' value='"+pendPur.purchaseId+"' /></td>"
	    		+ "<td>"+pendPur.invNo+"</td>"
	    		+ "<td>"+pendPur.invDate+"</td>"
	    		//+ "<td>"+pendPur.distributorName+"</td>"
	    		+ "<td>"+pendPur.billNo+"</td>"
	    		+ "<td>"+pendPur.billDate+"</td>";
		    	if($("#isMrpEnableFlag").val()==1)
				{
		    		row += "<td>"+pendPur.totalMrp+"</td>";
		    	}
	    		row+= "<td>"+pendPur.specDiscPer+"</td>"
	    		+ "<td>"+pendPur.netAmount+"</td>"
	    		+ "</tr>";
	    	$("#pendingPurchaseChallanBody").append(row);
		}
	}

}

function addCheckedPendingPurchase()
{
	idList = "";

	$("#pendingPurchaseChallanDiv input[name=purId]").each(function(){
		if($(this).is(":checked")) {
			idList += $(this).val() + ",";
		}
	});
	...
	if(!idList.length>0)
	{
		//alert("Please select some Purchase Challans to continue");

		$("#confirmval").val(-1);
		document.getElementById('confirmmessagecont').innerHTML = "Please select some Purchase Challans to continue";//getPurInvText.vendorChangeError;
		showConfirmModal();
	...
	var ajaxCallObject = new CustomBrowserXMLObject();
	ajaxCallObject.callAjax(BASE_URL + "/purchaseinvoice/getPurchaseDetailsbyPurchaseIDs/" + idList + ".htm", function(resp) {
		var obj = jQuery.parseJSON(resp);
		console.log("PurchaseDetailsbyPurchaseIDs : "+obj.length);
		//in loop  fillItemDetailsDiv(obj[0]);
		$("#peitem tbody").html("");
		for (var i = 0; i < obj.length; i++) {
			addInItemTable(obj[i]);
...
function addInItemTable(purdetobj)
{
	var obj = purdetobj;

	//$("#purHistoryDiv").addClass("hide");//open purchase history details popup
   /* console.log(JSON.stringify(purdetobj));*/
	var strng_batch_no="'"+obj.batchNo+"'";
	var strng_tbrow_id = "'"+obj.itemId+"_"+obj.batchNo+"'";
	var tr = '<tr id="tblrow_' + obj.itemId+'_'+obj.batchNo + '" style="cursor: pointer;">';
	var totmrp = (Number(obj.packQty) + Number(obj.freeQty)) * $("#mrp").val();
	var row1 = '<td id="tbl_item_name">' + obj.itemName + '</td>';
	var row2 = '<td id="tbl_batch_no">' + obj.batchNo + '</td>';
	//if($("#item_expiryDateRequired_req").val()==1){
	if(obj.expiryDate!='' && obj.expiryDate != undefined && obj.expiryDate != null) {
		if (isWholesale==1) {
   			var row3 = '<td id="tbl_exp">' + moment(obj.expiryDate).format('MM/YY') + '</td>';
   		}
   		else {
   			var row3 = '<td id="tbl_exp">' + moment(obj.expiryDate).format('YYYY-MM-DD') + '</td>';
   		}
	}else{
		var row3 = '<td id="tbl_exp">NA</td>';
	}
	//var row3 = '<td id="tbl_exp">' + $("#exp").val() + '</td>';
	var row4 = '<td id="tbl_pqty" class="numeric">' + obj.packQty + '</td>';
	var row7 = '<td id="tbl_free" class="numeric">' + obj.freeQty + '</td>';
	var previousFree = '<td id="tbl_prevfree" class="hide">0</td>';

	var row9 = '<td id="tbl_rate" class="numeric">' + parseFloat(obj.rate).toFixed(2) + '</td>';
	var row10 = '<td id="tbl_ed" class="numeric">' + obj.ed + '</td>';
	var row11 = '<td id="tbl_tax" class="numeric">' + obj.taxAmount + '</td>';
	var row12 = '<td id="tbl_vat" class="numeric">' + obj.vat + '</td>';
	var row13 = '<td id="tbl_disc" class="numeric">' + obj.disc + '</td>';
	var row14 = '<td id="tbl_amnt" class="numeric">' + parseFloat(obj.rate * obj.packQty).toFixed(2) + '</td>';
	//var net_amount = (Number($("#total").val()) + Number($("#tax").val())) - Number($("#disc").val());
	var net_amount = obj.purchaseNetAmount;
	var netamt = '<td id="tbl_netamt" class="numeric">' + parseFloat(net_amount).toFixed(2) + '</td>';
	var row16 = '<td><button class="btn btn-theme04 btn-xs" onclick="javascript:showPurItemDelModal('+strng_tbrow_id+');"><i class="fa fa-trash-o "></i></button></td>';
	var row17 = '<td id="tbl_ma" class="hide">' + ma + '</td>'; //????
	var row18 = '<td id="tbl_grp" class="hide">' + obj.grpId + '</td>';
	var row19 = '<td id="tbl_sch" class="hide">' + obj.schdId + '</td>';
	var row20 = '<td id="tbl_mfg" class="hide">' + obj.manuId + '</td>'; //$("#mfgid").val() ???
	var row21 = '<td id="tbl_edprcnt" class="hide">' + obj.edPer + '</td>';
	var row22 = '<td id="tbl_taxprcnt">' + obj.taxPercentage + '</td>';
	var taxid = '<td id="tbl_taxid" class="hide">' + obj.taxId + '</td>';
	var taxmode = '<td id="tbl_taxmode" class="hide">' + obj.taxMode + '</td>';
	var isgrptax = '<td id="tbl_isgrptax" class="hide">' + obj.isGroupTax + '</td>';
	var row23 = '<td id="tbl_vatprcnt" class="hide">' + obj.vatPer + '</td>';
	var row24 = '<td id="tbl_dprcnt">' + obj.discPer + '</td>'; //item_dprcnt ????
	var row25 = '<td id="tbl_id" class="hide">' + obj.id + '</td>'; //$("#id").val() ????
	var row26 = '<td id="tbl_itemid" class="hide">' + obj.itemId + '</td>';
	var row27 = '<td id="tbl_grpname" class="hide">' + obj.grpName + '</td>';
	var row28 = '<td id="tbl_schname" class="hide">' + obj.schdName + '</td>';
	var row29 = '<td id="tbl_mfgname" class="hide">' + obj.manuName + '</td>';
	var packunitid = '<td id="tbl_punitid" class="hide">' + obj.packUnitId + '</td>';


	var isFreeRow = '<td id="tbl_isFree" class="hide"></td>'; //????isFree isFree


	var ltadjRow = '<td id="tbl_ltAdj" class="hide">'+obj.itemLotAdjAmount+'</td>'; //???? ltadj
	var sku = '<td id="tbl_sku">' + obj.sku + '</td>';
	var hsn = '<td id="tbl_hsn" class="hide">' + obj.hsnCode + '</td>';
	var tbl_sale_rate = '<td id="tbl_saleRate" class="numeric">' + obj.saleRate + '</td>';
	var poId = '<td id="tbl_poid" class="hide">0</td>';
	var predprbfrspdp = '<td id="tbl_predprbfrspdp" class="hide">'+obj.discPer+'</td>';//item_dprcnt ????
	var billpackqty = "<td class='hide' id='purrettabpqtyhide'>0</td>";
	var mode = "<td class='hide' id='tbl_mode'>P</td>"; //?????
	var returnReason = "<td class='hide' id='tbl_ret_reason'></td>";
	// for common retail
	var expmonth = '<td id="tbl_expmonth" class="hide">' + obj.expiryMonth + '</td>';
	/*if(obj.expiryMonthRequired==1){
     var mfd = '<td id="tbl_mfd" class="hide">' + $("#mfd").val() + '</td>';
	}else{*/ //????
	 var mfd = '<td id="tbl_mfd" class="hide">NA</td>';
	//} ????
	...
	//var mfd = '<td id="tbl_mfd" class="hide">' + $("#mfd").val() + '</td>';
	var location = '<td id="tbl_location" class="hide">' + obj.locationId + '</td>';

	var row5 = '<td id="tbl_lqty" class="numeric hide">' + obj.looseQty + '</td>';
	var row6 = '<td id="tbl_ratio" class="numeric hide">' + obj.conversion + '</td>'; //$("#ratio").val() ????
	var row8 = '<td id="tbl_mrp" class="numeric hide">' + obj.mrp + '</td>';
	var row15 = '<td id="tbl_totamnt" class="numeric hide">' + obj.totAmount + '</td>'; //parseFloat(totmrp).toFixed(2) ??????
	var slnos = '<td id="tbl_slnos" class="numeric hide">' + $("#allVal").val() + '</td>';
	var slnoreq = '<td id="tbl_slnoreq" class="numeric hide">' + obj.serialNoRequired + '</td>';
	var item_batchWiseStock_req = '<td id="tbl_item_batchWiseStock_req" class="numeric hide">' + obj.batchWiseStock + '</td>';
	var item_expiryDateRequired_req = '<td id="tbl_item_expiryDateRequired_req" class="numeric hide">' + obj.expiryDateRequired + '</td>';
	var item_expiryMonthRequired_req = '<td id="tbl_item_expiryMonthRequired_req" class="numeric hide">' + obj.expiryMonthRequired + '</td>';
	var item_dualStockRequired_req = '<td id="tbl_item_dualStockRequired_req" class="numeric hide">' + obj.dualStockRequired + '</td>';
	var item_mrpRequired_req = '<td id="tbl_item_mrpRequired_req" class="numeric hide">' + obj.mrpRequired + '</td>';
	var item_locationRequired_req = '<td id="tbl_item_locationRequired_req" class="numeric hide">' + obj.locationRequired + '</td>';
	var item_priceListRequired_req = '<td id="tbl_item_priceListRequired_req" class="numeric hide">' + obj.priceListRequired + '</td>';
	//tr = tr + row1 + row2 + row3 + row4 + row5 + row6 + row7 + row8 + row9 + row10 + row11 + row12 + row13 + row14 + row15 + row16 + row17 + row18 + row19 + row20 + row21 + row22 + row23 + row24 + row25 + row26 + row27 + row28 + row29 + packunitid + isFreeRow + ltadjRow +'</tr>';
	tr = tr + row1 + sku + row2 + row3 + row4 + row7 + previousFree + row9 + tbl_sale_rate + row22 + row11 + row24 + row13 + row14 + netamt + row16 + row17 + row18 + row19 + row20 + taxid + taxmode + isgrptax + row25 + row26 + row27 + row28 + row29 + packunitid + isFreeRow + ltadjRow  + hsn + poId + predprbfrspdp + billpackqty + mode + returnReason+expmonth+mfd+location+ row5 + row6+ row8 + row15+slnos+slnoreq + item_batchWiseStock_req+item_expiryDateRequired_req+item_expiryMonthRequired_req+item_dualStockRequired_req+item_mrpRequired_req+item_locationRequired_req+item_priceListRequired_req+'</tr>';
	//$("#peitem tbody").prepend(tr);
