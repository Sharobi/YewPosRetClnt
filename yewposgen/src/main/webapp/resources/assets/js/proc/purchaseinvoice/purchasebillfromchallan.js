...
function targetActionPur(){
	...
	if(result==0){
		...
		if ($('input[name=printPurchase]').is(":checked"))
		{
			location.href = BASE_URL + "/purchaseinvoice/printPurInvoice.htm?backUrl=loadpurchaseinvoice&pid=" + result;
		}
		else
		{
...
function getmodordercheckeditemlist() {
	...
	else
	{
		if(count!= 0)
		{
			$("#alertmessagecont").text("Batch & Exp "+getFieldText.fieldReq);
			$('#purorderdetailModal').modal('show');
			//$("#peitemtbody").empty();
		}
		else
...
function addInItemTable(purdetobj)
{
	...
	if(obj.expiryDate!='' && obj.expiryDate != undefined && obj.expiryDate != null) {
		/*var mydate = new Date(obj.expiryDate);
	...
	var row14 = '<td id="tbl_amnt" class="numeric">' + parseFloat(obj.rate * obj.packQty).toFixed(4) + '</td>';
	//var net_amount = (Number($("#total").val()) + Number($("#tax").val())) - Number($("#disc").val());
