...
function postAllPI(){
	...
	if($.isEmptyObject(purchase.purchaseDetails)){
		...
		var ajaxCallObject = new CustomBrowserXMLObject();
		ajaxCallObject.callAjaxPost(BASE_URL + "/purinv/postallselectedpurchaseinvoicedetails.htm", purchase, function(response) {
			$('#pleasewaitModal').modal('hide');
...
function targetURL(){
	location.href = BASE_URL + '/purinv/purchaseinvoiceregistersearch.htm';
}
...
function printPurchaseInvoice(pid)
{
	var id=parseInt(pid);	
	//location.href = BASE_URL + "/purinv/printPurInvoice.htm?backUrl=purchaseinvoiceregistersearch&pid=" + id;
	location.href = BASE_URL + "/purchaseinvoice/printPurchaseInvoice.htm?backUrl=loadpurchaseinvoicedet&pid=" + id;/*Sayantan Mahanty added date-20/02/2020*/
}