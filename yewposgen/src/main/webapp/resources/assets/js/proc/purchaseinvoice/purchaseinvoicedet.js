...
function DoConfirmPurDel() {
	...
	var ajaxCallObject = new CustomBrowserXMLObject();
	ajaxCallObject.callAjaxPost(BASE_URL + "/purinv/deletepurchaseinvoice.htm", CommonRelsetmapperObj, function(response) {
...
function targetURL() {
	...
	else
	{
		...
		{
			location.href = BASE_URL + '/purchaseinvoice/loadpurchaseinvoicedet/'+$("#confirmval").val()+'.htm';
		}
...
//		$("#colour").val(0);
	...
	console.log("itemdetval.grpId"+itemdetval.grpId);
...
function postPurchaseInv(){
	...
	else
	{
		...
		if(puritemlength>0)
		{
			...
			var ajaxCallObject = new CustomBrowserXMLObject();
			ajaxCallObject.callAjaxPost(BASE_URL + "/purinv/postpurchaseinvdirindir.htm", CommonRelsetmapperObj, function(response) {
				$('#pleasewaitModal').modal('hide');
				if (response == '1') {
					$("#confirmval").val("post");
...
//			alert("length="+$("input:checkbox[class=chk]:checked").length);
	...
	$("#add_btn").click(function() {
		...
		//	var row6 = '<td id="tbl_ratio" class="numeric">' + $("#ratio").val() + '</td>';
			...
			var grpid = '<td id="tbl_grpid" class="hide">' + $("#grpid").val() + '</td>';//group id changed from 'tbl_grp' to 'tbl_grpid' 21-06-2019
			var schid = '<td id="tbl_sch" class="hide">' + $("#schid").val() + '</td>';
			var mfgid = '<td id="tbl_mfg" class="hide">' + $("#mfgid").val() + '</td>';
			...
			tr = tr + row1 + purbarcode + row2 + row3 + row4 + row5 + row6 + row7 + previousFree + row9 + tbl_sale_rate + row22 + row11 + taxTypeId + row24 + row13 + row14  + netamt + row16 + printAction + row17 + /*row19 + row20 +*/ taxid + taxmode + isgrptax + row25 + row26 +grpid+schid+mfgid + packunitid + isFreeRow + ltadjRow + sku + hsn + poId + predprbfrspdp + billpackqty + mode + returnReason + expmonth+mfd+location+ row8 + row15+slnos+slnoreq + item_batchWiseStock_req+item_expiryDateRequired_req+item_expiryMonthRequired_req+item_dualStockRequired_req+item_mrpRequired_req+item_locationRequired_req+item_priceListRequired_req+'</tr>';
		...
		if($("#itemid").val()==0 || $("#itemid").val()=="")
		{
			...
			$("#peitem tr").not('thead tr').each(function(	i,
				...
				if($(this).find('#tbl_mode').text()=="P")
				{
					...
					PurchaseDetailsObj.grpId = $(this).find('#tbl_grpid').text();//group id changed from 'tbl_grp' to 'tbl_grpid' 21-06-2019
...
function addForRet() {
	...
	if (itempresent == 1) {
		...
		tr = tr + row1 + sku + row2 + row3 + row4 + row5 + row6+ row7 + previousFree + row9 + tbl_sale_rate + row22 + row11 + row24 + row13 + row14 + netamt + row16 + printAction + row17 + row18 + row19 + row20 + taxid + taxmode + isgrptax + row25 + row26 + row27 + row28 + row29 + packunitid + isFreeRow + ltadjRow + hsn + poId + predprbfrspdp + billpackqty + mode + returnReason+expmonth+mfd+location+ row8 + row15+slnos+slnoreq + item_batchWiseStock_req+item_expiryDateRequired_req+item_expiryMonthRequired_req+item_dualStockRequired_req+item_mrpRequired_req+item_locationRequired_req+item_priceListRequired_req+chkdSlNos+'</tr>';
		$("#peitem tbody").prepend(tr);
...
function printByBarcode(itemid,itemname,sku,mrp,salerate,batch,exp,qty,storeName,category,subcategory,grp,size,color)
{
...
$(document).ready(function(){
	...
	console.log($("#purtype").is(":checked"));
	if(!$("#purtype").is(":checked"))
	{
		console.log("unchecked");
		idList = $("#purchaseIDList").val();
		...
		// var pinvid=$("#orderno").val();
		console.log("Invoice ID = "+$("#purchaseInvId").val());
		getpendingpurchasechallans(parseInt($("#purchaseInvId").val()));
		//removeAllItems();
	}
	else
	{
...
function getpendingpurchasechallans(invid)
{
	var ajaxCallObject = new CustomBrowserXMLObject();
	ajaxCallObject.callAjax(BASE_URL + "/purinv/getPurchaseChallanListByInvID/" + invid + ".htm", function(resp) {
		var obj = jQuery.parseJSON(resp);
...
function creatependingPurchaseChallanDetails(pendingPurchaseChallans){
	...
	for ( var i = 0; i < pendingPurchaseChallans.length; i++) {
		...
		var isChecked = "";
		console.log("Id present = "+(","+idList+",")+" comp with "+(","+pendPur.purchaseId+","));

		if((","+idList+",").includes(","+pendPur.purchaseId+","))
		{
			isChecked = " checked";
		}

		var row = "<tr>"
			+ "<td><input name='purId' type='checkbox' id='"+pendPur.invNo+"' value='"+pendPur.purchaseId+"' "+isChecked+" /></td>"
    		+ "<td>"+pendPur.invNo+"</td>"
...
function addInItemTable(purdetobj)
{
	...
	if($("#item_expiryDateRequired_req").val()==1){
		var row3 = '<td id="tbl_exp">' + obj.expiryDate + '</td>';
	}else{
		var row3 = '<td id="tbl_exp">NA</td>';
	}
	//var row3 = '<td id="tbl_exp">' + $("#exp").val() + '</td>';
	...
	var row16 = '<td><button class="btn btn-theme04 btn-xs" onclick="javascript:showPurItemDelModal('+strng_tbrow_id+');"><i class="fa fa-trash-o "></i></button></td>';
	var row17 = '<td id="tbl_ma" class="hide">' + ma + '</td>'; //????
	var row18 = '<td id="tbl_grpid" class="hide">' + obj.grpId + '</td>';//group id changed from 'tbl_grp' to 'tbl_grpid' 21-06-2019
	...
	var sku = '<td id="tbl_sku" class="hide">' + obj.sku + '</td>';
