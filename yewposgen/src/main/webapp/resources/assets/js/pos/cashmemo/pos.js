...
$(document).ready(function() {
	...
	$("#item_pqty").blur(function() {
		...
		if(item_is_slno==1){
			...
			//console.log("Case#1 itemexp=" + itemexp + " & itemexpdate=" + itemexpdate);
			...
			CommResultsetObj.saleId = $("#saleId").val();
			//console.log("Case#2 itemexp=" + itemexp + " & itemexpdate=" + itemexpdate);
...
function getClickeditmdet(clickitmdet) {
	...
	if (isWholesale!=0) // whole sale
    {
    	if(clickitmdet.batchNo!=null && clickitmdet.batchNo!=undefined)
    		getpurhistoryofitem(clickitmdet.itemId,clickitmdet.batchNo);
		else
			getpurhistoryofitem(clickitmdet.itemId,'NA');
	}
	//console.log("clickitmdet="+JSON.Stringify(clickitmdet));
	...
	///////copy
	//var itemexpdate = ($("#item_expDate").val().trim() == "")? "NA" : $("#item_expDate").val() ;
	if(clickitmdet.expiryDateFormat=="NA")
		{
		$("#item_expDate").val("NA");
		}
	else{
	 ...
	 ///$("#item_expDate").val(moment(clickitmdet.expiryDate).format('YYYY-MM-DD'));
	/*$("#item_expDate").val(clickitmdet.expiryDate);*/
...
//						createdrowline = starttrline + name + batchno + expiryDateFormat + manfacname + packQty + looseQty + conversion + mrppack + mrp + amt + rateperunit + vatperc + item_taxPercentage + vatamt + discperc + discamt + totamt + rowdelete + itembarcode + punitid + lunitid + content + stockedqty + mrpperunit + rate + schename + taxperc + taxamt + contentid + item_taxId + item_isGroupTax + item_discount + item_isDiscount + item_maxDiscountLimit + item_CalcTaxAmt + item_taxMode + item_hsnCode + item_purCost + item_purCostperUnit + salerate + item_purChase_rate+item_ltadj + endtrline;
							...
							createdrowline = starttrline + name + itembarcode + batchno + expiryDateFormat + manfacname + packQty + freetd +
...
function openAddModal(addSubjct) {
	...
	if (addSubjct == "cust") { // Customer add
		...
		$('#country').val(countryId+".0");/*Sayantan Mahanty added date-2/03/2020*/
