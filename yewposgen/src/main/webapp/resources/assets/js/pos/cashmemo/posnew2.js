...
$(document).ready(function() {
	...
	if ($("#tenderamt").val() > 0) {
		...
		$("#refundAmt").val(parseFloat($("#tenderamt").val() - $("#cashAmt").val()>0?$("#tenderamt").val() - $("#cashAmt").val():0).toFixed(2));
	} else {
	...
	$("#item_pqty").blur(function() {
		...
		if(item_is_slno==1){
			...
			ajaxCallObject.callAjaxPost(BASE_URL + "/stock/getserialnobyitemid.htm", CommResultsetObj, function(response) {
				...
				console.log("_serial no checking =" + response);
	...
	$("#item_dis").keyup(function() {
		...
		if (Number(item_dis) > Number(item_maxdisper)) {
			...
			$("#item_dis").val(parseFloat(0).toFixed(2));
	...
	//item_rate_ls change calculation $("#item_rate_ls").keyup(function()
	...
	/*	var item_salerate_ls = $(this).val();
		...
		total = total - discamt;
		$("#item_tot").val(parseFloat(total).toFixed(4));
		$("#item_rate_ls").val(parseFloat(item_rate_ls));
		vatamt = parseFloat(total * item_vat / 100).toFixed(4);
		$("#item_vatamt").val(parseFloat(vatamt).toFixed(4));
		taxamt = parseFloat(total * item_tax / 100).toFixed(4);
		$("#item_taxamt").val(parseFloat(taxamt).toFixed(4));
...
*/
	...
	$("#tenderamt").keyup(function() {
		if (isNaN($("#tenderamt").val())) {
			...
			// amt means tender amount
			if (amt > nettot) {
				//				$("#refundAmt").val(parseFloat(amt - nettot).toFixed(2));
				$("#refundAmt").val(parseFloat(amt - nettot).toFixed(2));
				//$("#refundAmt").val(parseFloat(amt - cashamt).toFixed(2));
			} else {
				$("#refundAmt").val(parseFloat(0).toFixed(2));
			}


			if (nettot == 0) {// when nettot= 0
	...
	$("#cashAmt").keyup(function() {
		if (isNaN($("#cashAmt").val())) {
			...
			/*if (cashamt > tenderamt) {
				$("#cashAmt").val(parseFloat(tenderamt).toFixed(2));
				cashamt = parseFloat(tenderamt).toFixed(2);
			}*/
			if (tenderamt>0) {
...
function getItemByManufacture(manuid) {
 ...
 //var itemsbymanufactureidtbl = $('#itemsbymanufactureidtbl').DataTable();
//	 itemsbymanufactureidtbl.destroy();
...
function calculateTotalltadj() {
	...
	if (totsaleltadj>0) {
		  getvendorledger_sale($('#lot_adjas_codef').val(),0,0,7);// for lot adjasment
...
/*
 * for final nettotal calculation
 */
function calculateNetTotal() {
	var grandNetTotal = 0.00;

	$('#selitem tbody tr').each(function() {
		var itmmrp = $(this).find("#saletabtotamt").html(); // item wise nettotal
		if ($(this).find("#saletabisfreeitemaginstitem").html()==0) {// IAI ignor when 0
	...
	if (grandNetTotal<=0) {
	 ...
	 if (!isEmpty($("#otheradjasment").val())) {
			// other adjamsment
		  grandNetTotal=parseFloat(grandNetTotal)+parseFloat($("#otheradjasment").val());
		   }
...
function calSpclDisNew() {
	...
	$('#selitem tbody tr').each(function() {
		...
		if(((item_free % 1)!=0) || item_free<1)
		{
			...
			$("#selitem tr#" + itemid).find('#saletabitemltadj').text(parseFloat(ltadj).toFixed(2));
...
function paySaleInvModal() {
	...
	//alert($("#paymodnettot").val());
	//$("#creditAmt").val($("#paymodnettot").val());
	var samepage_retamtval = $("#retamtvalsamepage").val();
	//alert("s="+s);
	if ($("#retamtvalsamepage").val() == undefined || $("#retamtvalsamepage").val() == "") {
		samepage_retamtval = 0;
	}
	if ($("#retamtsamepage").val() == 0) {

		if (Number($("#nettot").val()) > Number(samepage_retamtval)) {
			...
			if (creditcustomer == 0) // check credit customer or not
			{
				...
				if (samepage_retamtval==0) {// when not present on bill return then s is zero
					$("#cashAmt").val( parseFloat( $("#paymodnettot").val()  -  $("#payretadjamt").val() ).toFixed(2));
				}else {
				...
				$("#cashAmt").val(parseFloat(0.0).toFixed(2));

				if (samepage_retamtval==0) {// when not present on bill return then s is zero
					$("#creditAmt").val(parseFloat($("#paymodnettot").val()-$("#payretadjamt").val()).toFixed(2));
				}
				else {
					$("#creditAmt").val(parseFloat($("#paymodnettot").val()-samepage_retamtval).toFixed(2));
					$("#payretadjamt").val(parseFloat(s).toFixed(2));
				}
		...
		/*	if ($("#salecustname").val() == "" || $("#saledocname").val() == "") {
			...
			var idss=parseInt($("#salecustid").val());
...
function paySaleInv() {
	...
	if ($("#item_id").val() == 0 || $("#item_id").val() == "") {
		...
		sale.lotAdjAmountId=$('#lotAdjasment_ledger_idf').val();
	    sale.othAdjAmountId=$('#otherAdjasment_ledger_idf').val();
		//for same page return
		var cval = $("#confirmvalrsp").val();
...
function getClickeditmdet(clickitmdet) {
	...
	$("#item_code").val(clickitmdet.itemCode);//New Added 22.5.2019
	...
	//	alert(isexclu);
	if (salerate == 0) {
		if (isexclu == 2) {// for stor isexclu == 1
...
//	$("#item_lqty").val($("#selitem tr#" + itemid).find('#saletablqty').text());
	$("#item_free").val($("#selitem tr#" + itemid).find('#saletabfree').text());
	...
	$("#item_code").val($("#selitem tr#" + itemid).find('#saletabcode').text()); // new added 22.5.2019
...
var isfreeIAI= $('#isfreeiai').is(":checked")?1:0;
	...
	//var curstkcheck=$("#curstkcheck").val();
	//alert("curstkcheck="+curstkcheck);
	if (Validation() == 1) {
		return false;
	} else {
		document.getElementById('alertMsg').innerHTML = "";
		...
		ajaxCallObject.callAjaxPost(BASE_URL + "/stock/getcurrstockbybatexpmrp.htm", CommResultsetObj, function(response) {
			...
			for ( var i = 0; i < itemcurstkdets.length; i++) {
				...
				//	var calamt = (($('#item_pqty').val() * $('#item_conv').val()) + Number($('#item_lqty').val())) * $('#item_rate_ls').val();
					...
					/*===========================Note=======================
					 * EnableBatch,EnableExp find in new pos jsp
					 */
					if (EnableBatch==1) {
						var batchno = "<td id='saletabbat'>" + $('#item_batch').val() + "</td>";
					}else {
						var batchno = "<td id='saletabbat' class='hide'>" + $('#item_batch').val() + "</td>";
					}
					if (EnableExp==1) {
						var expiryDateFormat = "<td id='saletabexpdt'>" + $('#item_exp').val() + "</td>";
					}else
						{
						var expiryDateFormat = "<td id='saletabexpdt' class='hide'>" + $('#item_exp').val() + "</td>";
						}
...
//					var looseQty = "<td id='saletablqty'>" + lqty + "</td>";
					if(isFreeFlag==1){
						freetd = "<td id='saletabfree'>" + free + "</td>";
					}else{
						freetd = "<td id='saletabfree' class='hide'>" + free + "</td>";
					}
					if(isConversionFlag==1){
					 conversion = "<td id='saletabconv'>" + $('#item_conv').val() + "</td>";
					}else{
					 conversion = "<td id='saletabconv' class='hide'>" + $('#item_conv').val() + "</td>";
					}
					if(isMrpEnableFlag==1){
					 mrppack = "<td id='saletabmrppack'>" + $('#item_mrp_pack').val() + "</td>";
					 mrp = "<td id='saletabmrp'>" + parseFloat(mrpamt).toFixed(4) + "</td>";
					}else{
					 mrppack = "<td id='saletabmrppack' class='hide'>" + $('#item_mrp_pack').val() + "</td>";
					 mrp = "<td id='saletabmrp' class='hide'>" + parseFloat(mrpamt).toFixed(4) + "</td>";
					}
					...
					var item_taxPercentage = "<td class='hide' id='saletabtaxPercentage'>" + parseFloat($('#item_taxPercentage').val()).toFixed(4) + "</td>";
					var discperc = "<td id='saletabdiscperc'>" + parseFloat(isfreeIAI==0?disc:0).toFixed(4) + "</td>";
					var totamt = "<td id='saletabtotamt'>" + parseFloat(isfreeIAI==0?$('#item_tot').val():0).toFixed(4) + "</td>";
					...
					var itemcode = "<td id='saletabcode' class='hide'>" + $('#item_code').val() + "</td>"; // new added 22.5.2019
					var endtrline = "</tr>";
...
//						$("#selitem tr#" + itemid).find('#saletablqty').text(lqty);
						...
						$("#selitem tr#" + itemid).find('#saletabrateperunit').text(parseFloat(isfreeIAI==0?$("#item_rate_ls").val():0));
						$("#selitem tr#" + itemid).find('#saletabrate').text(parseFloat($("#item_rate_ls_hid").val()));
						...
						$("#selitem tr#" + itemid).find('#saletabtotamt').text(parseFloat(isfreeIAI==0?$("#item_tot").val():0));
						...
						$("#selitem tr#" + itemid).find('#saletabdualstock').text(item_dual_stock);
						...
						$("#selitem tr#" + itemid).find('#saletabcode').text($('#item_code').val()); // new added 22.5.2019
					} else {
//						createdrowline = starttrline + name + batchno + expiryDateFormat + manfacname + packQty + looseQty + conversion + mrppack + mrp + amt + rateperunit + vatperc + item_taxPercentage + vatamt + discperc + discamt + totamt + rowdelete + itembarcode + punitid + lunitid + content + stockedqty + mrpperunit + rate + schename + taxperc + taxamt + contentid + item_taxId + item_isGroupTax + item_discount + item_isDiscount + item_maxDiscountLimit + item_CalcTaxAmt + item_taxMode + item_hsnCode + item_purCost + item_purCostperUnit + salerate + item_purChase_rate+item_ltadj + endtrline;
						...
						createdrowline = starttrline + name + batchno + expiryDateFormat + manfacname + packQty + freetd +
						conversion + mrppack + mrp + rateperunit + amt + vatperc + item_taxPercentage +
						vatamt + discperc + discamt + totamt + rowdelete + itembarcode + punitid + lunitid +
						content + stockedqty + mrpperunit + rate + schename + taxperc + taxamt + contentid +
						item_taxId + item_isGroupTax + item_discount + item_isDiscount + item_maxDiscountLimit +
						item_CalcTaxAmt + item_taxMode + item_hsnCode + item_purCost + item_purCostperUnit + salerate
						+ item_purChase_rate+item_ltadj+slnos+isslnos+slnoschkid+item_taxtype+saletabexpdate+itemDualStock
						+salasManIsPerAmt+isfreeItemagaistitem + itemcode + endtrline;
...
function okRetAdjMod() {
	...
	if(creditcustomer==0){
		...
		//$("#cashAmt").val(parseFloat(Number(nettot) - Number(totretadjamt)).toFixed(2));
		$("#creditAmt").val(parseFloat(Number(nettot) - Number(totretadjamt)).toFixed(2))
	}
	//$("#cashAmt").val(parseFloat(Number(nettot) - Number(totretadjamt)).toFixed(2));
...
function addEditCustomer() {
	...
	if (fieldValidationWithAlertDivId(field_names, "custAddAlertMsg") > 0) {
		...
		//console.log(JSON.stringify(CustomerMasterObj));
...
function calculateDefaultvalue(){
   $('#alertMsg').html('');
		...
		if (item_rate_ls==0) {
   			autoadd_barcodeflag=0;
   			    //$('#alertMsg').html('Sale rate Requied: ' );
   			   // return ;
		 ...
		 */
		if (sales_mans_id>0) {
			$("#salesman_amt").val(parseFloat( ( total*sales_mans_per ) / 100 ).toFixed(4))
		...
		if(((item_free % 1)!=0) || item_free<1)
		{
			...
			$("#item_ltadj").val(parseFloat(ltadj).toFixed(4));
			// LtAdj calculation end
			total=parseFloat(totalret-ltadj).toFixed(4);
			discamt = (total) * item_dis / 100;

			$("#item_discamt").val(parseFloat(discamt).toFixed(4));
			total=parseFloat(total-discamt).toFixed(4);
...
//$('#pleasewaitModal').modal('show');
	...
	if (para==7) {// for lotadjasment
		console.log("for lotadjasment ");
		$.each(status, function(i) {
			// $('#cash_ledger_html1').html("Dr-"+status[i].name );
	...
	if (para==8) {// for lotadjasment
		console.log("for other adjasment ");
		$.each(status, function(i) {
			// $('#cash_ledger_html1').html("Dr-"+status[i].name );
			 $('#otherAdjasment_ledger_idf').val(status[i].id);
...
function loadsaleman()
{
	 ...
	 */
	     ...
	     var ajaxCallObjectret = new CustomBrowserXMLObject();
				ajaxCallObjectret.callAjax(BASE_URL + "/pos/loadallsaleman.htm", function(res) {
					var salsemanlist = JSON.parse(res);
						//console.log(JSON.stringify(salsemanlist));

						  var str = "<option value='0'>Select....</option>";
				            $.each(salsemanlist, function(i) {

				            	if (salsemanlist[i].commissionPercentage>0) {
				            		var decimal = salsemanlist[i].commissionPercentage - Math.floor(salsemanlist[i].commissionPercentage);


				            		if (decimal==0) {
				            			str = str + "<option value=" + salsemanlist[i].id+"_"+ salsemanlist[i].commissionPercentage+".0"+ ">" + salsemanlist[i].name + "</option>";
									}else
										{
										str = str + "<option value=" + salsemanlist[i].id+"_"+ salsemanlist[i].commissionPercentage+ ">" + salsemanlist[i].name + "</option>";
										}


								}else {

									var decimal2=defaul_sales_man_per - Math.floor(defaul_sales_man_per);
									if (decimal2==0) {
										str = str + "<option value=" + salsemanlist[i].id+"_"+ defaul_sales_man_per+ ".0"+">" + salsemanlist[i].name + "</option>";
									}else {
										str = str + "<option value=" + salsemanlist[i].id+"_"+ defaul_sales_man_per+ ">" + salsemanlist[i].name + "</option>";
									}

								}

				            });
				            $("#salemanslist").html(str);
				});
...
$("#otheradjasment").keyup(function() {
	...
	if ($("#otherAdjasment_ledger_idf").val()==0) {
		 getvendorledger_sale($('#other_adjas_codef').val(),0,0,8);// for lot adjasment
	}
