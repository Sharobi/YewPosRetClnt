		...
		<div class="col-lg-12 col-md-12 col-sm-12">
			<div class="panel-trx panel-default">
				<div class="panel-body-trx">
					<form modelAttribute="commonResultSetMapper" action="${pageContext.request.contextPath }/purorder/searchpurchaseorderregister.htm" method="post" id="searchForm">
						<table>
							...
							<tr>
								...
								<td width="45px;" style="padding: 0 1px;"><input class="form-control-trx" type="text" style="padding: 4px 1px;" id="retmemoDoc" value="${poDefaultPrefix}/" size="4" readonly></td>
				...
				<tbody>
					<c:if test="${!empty purchaseOrderList }">
						<c:forEach items="${purchaseOrderList}" var="purchaseOrder">
							<tr>
								<td>${purchaseOrder.invNo}</td>
								<td><fmt:parseDate value="${purchaseOrder.invDate}" var="parsedInvDate" pattern="MMM dd, yyyy" /> <fmt:formatDate pattern="yyyy-MM-dd" value="${parsedInvDate}" /></td>
								<td>${purchaseOrder.distributorName}</td>
								<td>${purchaseOrder.grossAmount}</td>
								<td>${purchaseOrder.totalMrp}</td>
								<td>${purchaseOrder.netAmount}</td>
								<td>${purchaseOrder.remarks}</td>
								<td><c:if test="${purchaseOrder.status==1}">
								Partially Completed
								</c:if>
								<c:if test="${purchaseOrder.status==0}">
								Pending
								</c:if>
								<c:if test="${purchaseOrder.status==2}">
								Completed
								</c:if>
								<c:if test="${purchaseOrder.status==3}">
								Closed
								</c:if>
								</td>
								<td><c:if test="${purchaseOrder.isPosted==1}">
								Posted
								</c:if>
									<c:if test="${purchaseOrder.isPosted==0}">
								Unposted
								</c:if></td>
								<td>
								<button class="btn btn-success btn-xs" onclick="window.location.href ='${pageContext.request.contextPath}/purorder/reprintmemo/purorder.htm?reprint=N&backUrl=purorder/loadpurordregister&purId=3465fg-trw73sxz-${purchaseOrder.purchaseOrderId}-utew09-qdd55-4320jhhgrt'">
									<i class="fa fa-print"></i>Print</button>
								<c:if test="${purchaseOrder.isPosted==1}">
										<button class="btn btn-success btn-xs" onclick="window.location.href='${pageContext.request.contextPath}/purorder/loadpurorderdet/${purchaseOrder.purchaseOrderId}.htm'"><spring:message code="cmn.jsp.btn.view" text="View" /></button>
										<c:if test="${purchaseOrder.status!=3}">
											<button class="btn btn-warning btn-xs" onclick="closePurchaseOrder(${purchaseOrder.purchaseOrderId})"><spring:message code="cmn.jsp.btn.close" text="Close" /></button>
										</c:if>
									</c:if>
									<c:if test="${purchaseOrder.isPosted==0}">
										<c:if test="${menuByUserDTO.isAll==1}">
											<button class="btn btn-theme btn-xs" type="button" onclick="postPurchaseInv(${purchaseOrder.purchaseOrderId})">Post</button>
											<button class="btn btn-info btn-xs" type="button" onclick="window.location.href='${pageContext.request.contextPath}/purorder/loadpurorderdet/${purchaseOrder.purchaseOrderId}.htm'">
												<i class="fa fa-pencil"></i>
												<spring:message code="cmn.jsp.btn.edit" text="Edit" />
											</button>
										</c:if>
										<c:if test="${menuByUserDTO.isView==1}">
											<button class="btn btn-theme04 btn-xs" type="button" onclick="deletePurchaseOrder(${purchaseOrder.purchaseOrderId})">
...
<!-- Confirm Modal end -->
<script src="${pageContext.request.contextPath }/assets/js/proc/purorderreg/purorderreg.js"></script>
<c:if test="${pageContext.response.locale == 'en'}">
	<script src="${pageContext.request.contextPath}/assets/js/proc/purorderreg/purorderreg_en.js"></script>
