...
<section class="wrapper">
	<div class="row">
		<div class="col-lg-12">
			<p>
				<spring:message code="stockrecv.reg.jsp.title"
					text="Stock Receive Register..." />
			</p>
		</div>
			...
			<div class="panel-trx panel-default">
				<div class="panel-body-trx">
					<form modelAttribute="commonResultSetMapper"
						action="${pageContext.request.contextPath }/stock/searchstockreceiveregister.htm"
					...
					<c:if test="${!empty stockTransferDTO }">
						<c:forEach items="${stockTransferDTO}" var="stockTransfer">
							<tr>
								...
								<td>${stockTransfer.recvdNetAmount}</td>
								<td>${stockTransfer.receiveRemarks}</td>
								...
								<td><c:if test="${stockTransfer.recvIsPosted==0}">
										<c:if test="${stockTransfer.dispatchStatus=='Y'}">
											<c:if test="${menuByUserDTO.isAll==1}">
												<c:if test="${stockTransfer.receiveStatus=='N'}">
													<button class="btn btn-success btn-xs" type="button"
														onclick="window.location.href='${pageContext.request.contextPath}/stock/stockreceiveheader/${stockTransfer.transId}.htm'">
														<i class="fa fa-pencil"></i>
														<spring:message code="pos.jsp.receivestock" text="Receive" />
													</button>
												...
												<c:if test="${stockTransfer.receiveStatus=='Y'}">
													<button class="btn btn-info btn-xs" type="button"
														onclick="window.location.href='${pageContext.request.contextPath}/stock/stockreceiveheader/${stockTransfer.transId}.htm'">
														<i class="fa fa-pencil"></i>
														<spring:message code="pos.jsp.editstock" text="Edit" />
													</button>
													<button type="button" class="btn btn-warning btn-xs"
														onclick="closeStockTransfer(${stockTransfer.transId})">
														<spring:message code="pos.jsp.closestock" text="Close" />
													</button>
												</c:if>
											</c:if>
										</c:if>
									</c:if> <c:if test="${stockTransfer.recvIsPosted==1}">
										<button class="btn btn-primary btn-xs" type="button"
											onclick="window.location.href='${pageContext.request.contextPath}/stock/stockreceiveheader/${stockTransfer.transId}.htm'">
											<spring:message code="pos.jsp.viewstock" text="View" />
										</button>
									    </c:if>									
									</td>
       							</tr>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button type="button" onclick="DoConfirmCloseStkRec()"
...
<!-- Confirm Modal end -->
<script
	src="${pageContext.request.contextPath }/assets/js/inventory/stock/stock_receive.js"></script>
