...
<!--
.ui-autocomplete {
	overflow-y: scroll; max-height: 250px; width: 150px; word-break: break-all;
}
...
<section class="wrapper">
	<div id="rowmarg" class="row">
		...
		<input type="hidden" id="retamtsamepagesaleid" value="${stockTransferHeader.transId}"/>
		...
		<div class="bs-example bs-example-tabs" data-example-id="togglable-tabs">
			<ul class="nav nav-tabs" id="myTabs" role="tablist">
				<li role="presentation" class="active "><a href="#home" id="home-tab" role="tab" data-toggle="tab" aria-controls="home" aria-expanded="true" style="font-weight: bolder;">Stock Receive</a></li>
...
<!-- 												</div> -->
											...
											<td style="padding: 0 1px;width:4%;"><input class="form-control-trx" type="text" style="padding: 4px 1px;" id="purOrderDoc" value="${soDefaultPrefix}/" size="4" readonly></td>
											<td style="width:4%;"><input class="form-control-trx" type="text" style="padding: 4px 1px;" id="purOrderFinyr" value="${sessionScope.sesloggedinUser.finyrCode}" size="5"></td>
											...
											<td style="padding: 0 1px;" class="hide"><input class="form-control-trx" type="text" id="saleinvno" readonly="readonly" value=""><input type="hidden" id="saleId" value="${stockTransferHeader.transId}"></td>
											...
											<td  width="${sessionScope.sesloggedinUser.companyId == 18? '17%' : '17%'}" style="padding: 0 1px;"><input class="form-control-trx" id="salecustph" placeholder="Search by phone no " type="text"><input type="hidden" id="salecustid" value="0"><input type="hidden" id="custCreditLimit" /><input type="hidden" id="custEmail" /></td>
										   <%--  <td  class= "${sesloggedinStore.isCustomerDisplay==1?' ': 'hide'}" width="${sessionScope.sesloggedinUser.companyId == 18? '20%' : '25%'}" style="padding: 0 1px;"><input class="form-control-trx" placeholder="Search by name " id="salecustname" type="text"></td> --%> 
											 <td  width="${sessionScope.sesloggedinUser.companyId == 18? '20%' : '25%'}" style="padding: 0 1px;"><input class="form-control-trx" placeholder="Search by name " id="salecustname" type="text"></td>
											...
											<c:choose>
											  <c:when test="${sessionScope.sesloggedinUser.companyId == 18}">
													    ...
													    <td id="add_doc_td">
														<button class="btn btn-primary" type="button" onclick="openAddModal('tailor')">
															<i class="fa fa-plus"></i>
														</button>
													    </td>
											  ...
											  <c:otherwise>
											       <td width="15%" style="padding: 0 1px;" class="hide"><input class="form-control-trx" id="saledocname" type="text"><input type="hidden" id="saledocid" value="0"></td>
											       <td id="add_doc_td" class="hide">
														<button class="btn btn-primary" type="button" onclick="openAddModal('doc')">
															<i class="fa fa-plus"></i>
														</button>
									...
									<c:if test="${stockTransferHeader.transId!=null}">
										<tr align="center" >
										<td><spring:message code="pos.jsp.trno" text="Transfer No" /></td>
											...
											<c:choose>
											  <c:when test="${sessionScope.sesloggedinUser.companyId == 18}">
											    <td class=""><spring:message code="pos.jsp.tailor" text="Tailor Name" /></td>
											</c:when>
											<c:otherwise>
											   <td class="hide"><spring:message code="pos.jsp.dctrName" text="Doc. Name" /></td>
											</c:otherwise>
										...
										<td class="hide"><input type="text" class="form-control-trx" id="date"  readonly value="<fmt:formatDate pattern="yyyy-MM-dd" value="${today}" />"></td>
										<c:if test="${stockTransferHeader.transId==null}">
											<td style="padding: 0 1px;">
												<div class="input-group">
													
													<input type="text" class="form-control-trx" id="tdate"  readonly value="<fmt:formatDate pattern="yyyy-MM-dd" value="${today}" />">
												</div>
											</td>
											...
											<td style="padding: 0 1px;">
												<div class="input-group">
													...
													<input type="text" class="form-control-trx" id="duedate" value="" readonly>
													<input type="hidden" id="duedate" value="${noOfDue}" />
												</div>
											</td>
										...
										<td style="padding: 0 1px;">
												<div class="input-group">																																							
												<select class="form-control-trx" id="fromTrans" disabled="true">
 												 <c:forEach items="${allstoresdata}" var="store">
 												<%--  <option value="${stockTransferHeader.fromStoreId}">${stockTransferHeader.fromStoreName}</option> --%>
									 ...
									 <c:if test="${sesloggedinUser.isAdmin == 1}">	
											...
											<td style="padding: 0 1px;">
                                                ...
                                                <input class="form-control-trx" type="text" id="transNo" value="${stockTransferHeader.stockTransNo}" readonly>
											...
											<td style="padding: 0 1px;">
												<div class="input-group">
							                       ...
							                       <input class="form-control-trx" type="text" id="tdate" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${parsedInvDate}" />" readonly>
												</div>
											</td>
									
											<td style="padding: 0 1px;">
												<div class="input-group">
												<fmt:parseDate value="${stockTransferHeader.dueDate}" var="parseddDate" pattern="MMM dd, yyyy" />
                                                <input class="form-control-trx" type="text" id="duedate" value="<fmt:formatDate pattern="yyyy-MM-dd" value="${parseddDate}"/>" readonly>
										...
										<td style="padding: 0 1px;">
											...
											<td style="padding: 0 1px;">
												<div class="input-group">													
													<select class="form-control-trx" id="toTrans" disabled="true">
												<option value="${stockTransferHeader.toStoreId}">${stockTransferHeader.toStoreName}</option> 									 		    												
												</select>
												</div>
											</td>
						...
						<div id="header_div_panel" class="panel-trx panel-default">
							<div class="panel-body-trx" id="header_div">
								...
								<table  >
									<tr align="center" >
										<c:choose>
										  <c:when test="${sessionScope.sesloggedinUser.primarySearchType == 1}">
										  ...
										  <c:when test="${sessionScope.sesloggedinUser.primarySearchType == 2}">
										      <td><spring:message code="cmn.jsp.barcode" text="Barcode" /></td>
										      <td><spring:message code="purinvdet.jsp.itemname" text="Item Name" /></td>
										     <%--  <td><spring:message code="item.jsp.code" text="Item Code" /></td> --%>
										  </c:when>
										...
										<td><spring:message code="pos.jsp.salerate" text="Sale Rate" /></td>
									...
									<tr>
												  ...
												  <td class="hide"  width="10%"><input class="form-control-trx" type="text" id="item_code" ></td>	 
												    <td width="25%" style="padding: 0 5px;"><input class="form-control-trx" type="text" id="item_name" readonly>
												     <input type="hidden" id="item_id" value="0"></td>
											      <td width="16%"><input class="form-control-trx" type="text" id="item_barcode" readonly></td>
											    
											  
										<td style="padding: 0 5px;" width="7%"><input class="form-control-trx" type="text" id="item_pqty" value="0" onkeydown="numcheck(event)" > 
										<input type="hidden" id="item_packunitid" value="0">
										<input type="hidden" id="item_is_slno" value="0"><input type="hidden" id="item_taxtype" value="0"></td>
										
										
											<!-- <td  width="7%" style="padding: 0 5px; "><input class="form-control-trx " type="text" id="current_stock" value="0" readonly></td> -->
											<td  width="7%" style="padding: 0 5px; " class="hide"><input class="form-control-trx " type="text" id="current_stock" value="0" readonly></td>
										<td style="display: none;"><input class="form-control-trx " onkeydown="numcheck(event)" type="hidden" id="item_free" value="0"><input class="form-control-trx hide" type="text" id="item_lqty" value="0"><input type="hidden" id="item_looseunitid" value="0"> <input type="hidden" id="item_stockedqty" value="0"><input type="hidden" id="item_ltadj" value="0"></td>
										<td style="padding: 0 5px;"><input class="form-control-trx" type="text" id="item_purChase_rate" readonly><input type="hidden" id="item_dualStockRequired_req" value="0" /></td>
										<td style="padding: 0 5px;"><input class="form-control-trx" type="text" id="item_sale_rate" readonly></td>
										...
										<td style="padding: 0 5px;" class="hide"><input class="form-control-trx" type="text" id="item_rate_ls" value="0" onkeydown="numcheck(event)"  ><input type="hidden" id="item_rate_ls_hid" value="0"></td>
										
										<td style="padding: 0 5px;" class="hide"><input class="form-control-trx" type="text" id="item_sale_rate_ls" value="0" readonly><input type="hidden" id="sale_rate_ls_hid" value="0" readonly></td>
										<td  width="9%" style="padding: 0 5px;" id="item_dis_td"><input class="form-control-trx" type="text" id="item_dis" onkeydown="numcheck(event)" value="0.0000" readonly><input type="hidden" id="item_discamt" value="0"><input type="hidden" id="item_mrpRequired_req" value="0"><input type="hidden" id="item_priceListRequired_req" value="0"><input type="hidden" id="item_expDate" tabindex="-1" readonly></td>
										...
										<td style="padding: 0 5px;" class="hide"><input class="form-control-trx" type="text" id="item_receive_qty" tabindex="-1" readonly></td>
										...
										<td style="padding: 0 5px;" class="hide"><input class="form-control-trx" type="text" id="transit_item" value="0">
										<input type="hidden" id="transit_item_hid" value="0"></td>
									</tr>
									<tr align="center" >
									...
									<tr>
										...
										<%--  <c:if test="${sessionScope.sesloggedinUser.companyId == 18}">  --%>
										  <!-- <td id="itmremarks_td"><input class="form-control-trx" type="text" value="" id="itmremarks"></td> -->
										<%--  </c:if> --%> 
											<td style="display: none;" id="ratio_val" class="hide"><input class="form-control-trx" type="text" id="item_conv" tabindex="-1" readonly></td>
										...
										<input type="hidden" id="item_saleorderid" value="0" />
										</td>
							...
							<c:if test="${stockTransferHeader.recvIsPosted==0}">
							 <c:if test="${stockTransfer.receiveStatus!='Y'}">
											<%--  <c:if test="${saleHeaderDTO.holdFlag ==0}">  --%>
												<td colspan="3" align="middle"><button class="btn btn-success btn-sm" title='Add into table ' type="button" onclick="addOrUpdateItemToDetailsTable(0)" id="add_btn">
													...
													<!-- <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#purModal">PURCHASE</button> -->
												</td>
											<%-- </c:if>   --%>
										</c:if> 
										</c:if>
							...
							<table id="selitem" style="margin-bottom: 5px;" class="table table-bordered table-striped table-condensed-trx table-hover">
								<thead>
									<tr>
										...
										<th><spring:message code="purinvdet.jsp.itembarcode" text="Barcode" /></th>
										...
										<th class= "${sesloggedinStore.isManufacturer==1?' ': 'hide'}"><spring:message code="purinvdet.jsp.mfg" text="Mfg" /></th>
										 ...
										 <th><spring:message code="pos.jsp.stocktransferjsp.receiveqty" text="Received Qty" /></th> 										  
											<th class="numeric"><spring:message code="pos.jsp.stocktransferjsp.sentqty" text="Send Qty" /></th>
											<%-- <th><spring:message code="pos.jsp.stocktransferjsp.transitqty" text="Transit Qty" /></th>  --%>
										...
										<th class="numeric"><spring:message code="pos.jsp.salerate" text="Sale Rate" /></th>
								...
								<tbody id="saletabitemdetails">
									...
									<c:if test="${!empty stockTransferDetails }">
										...
										<c:forEach items="${stockTransferDetails}" var="stockTransferDetail">
										
											<tr id="${stockTransferDetail.itemUniqueKey}" class='schxfreeiai ' style='cursor: pointer;' onclick="javascript:itemHeaderDivView(this.id);">
											...
											<td class="${sesloggedinStore.isManufacturer==1?' ': 'hide'}" id="saletabmanname">${stockTransferDetail.manufacturerName}</td>
											
											<fmt:parseDate value="${stockTransferDetail.expiryDate}" var="formattedExpDate" pattern="MMM dd, yyyy" />
											<td  class="hide" id="saletabexpdate"><fmt:formatDate pattern = "yyyy-MM-dd" value = "${formattedExpDate}" /></td>
											<td class="numeric hide" id="saletabcurrentstk">${stockTransferDetail.recvdPackQty}</td>
											<td class="numeric hide" id="saletabstkrecdetaisid">${stockTransferDetail.transDetailId}</td>
											<td class="numeric" id="saletabpqty">${stockTransferDetail.recvdPackQty}</td>
											
											<c:if test="${stockTransferHeader.dispatchStatus =='Y'}">
											
											<td class="numeric" id="saletabreceivepqty">${stockTransferDetail.sendPackQty}</td>
											<%-- <td class="numeric" id="saletabtransitpqty" class="hide">${stockTransferDetail.transitPackQty}
											
											</td> --%>
											</c:if>
										   <%--  <td class="numeric ${userinfo.isFree==1 ?'':'hide'}" id="saletabfree">${stockTransferDetail.freeQty}</td> --%>

									        <td class="hide" id="saletablqty">${stockTransferDetail.recvdLooseQty}</td>
											<%-- <td class="${sesloggedinStore.isFree==1?' ': 'hide'}" id="saletablqty">${saleDetail.looseQty}</td> --%>

											<%-- <td class="${sesloggedinStore.isConversion==1?' ': 'hide'}" id="saletabconv">${stockTransferDetail.conversion}</td> --%>
											<td class="hide" id="saletabconv">${stockTransferDetail.conversion}</td>

											<c:if test="${sessionScope.sesloggedinUser.isMrpEnable==1}">
											<td class="numeric" id="saletabmrppack"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.mrp}" /></td>
											<td class="numeric" id="saletabmrp"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.mrpPerUnit}" /></td>
											</c:if>
											<c:if test="${sessionScope.sesloggedinUser.isMrpEnable==0}">
											<td class="numeric hide" id="saletabmrppack"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.mrp}" /></td>
											<td class="numeric hide" id="saletabmrp"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.mrpPerUnit}" /></td>
											</c:if>
											...
											<td class="numeric" id="saletabamt"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${(stockTransferDetail.recvdPackQty*stockTransferDetail.ratePerUnit)}" /></td>
											<td class="numeric hide" id="sndsaletabamt"><fmt:formatNumber type="number" value="${(stockTransferDetail.recvdPackQty*stockTransferDetail.ratePerUnit)}" /></td>
                                       ...
                                       <%--   <c:if test="${saleDetail.itemdualstock==1}">
                                         <td class="numeric" id="saletabamt"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${(saleDetail.packQty*saleDetail.ratePerUnit)}" /></td>
  										...
  										<td class="numeric" id="saletabamt"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${((saleDetail.packQty*saleDetail.conversion)+saleDetail.looseQty)*saleDetail.ratePerUnit}" /></td>
											...
											<td class="numeric" id="saletabrateperunit"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.ratePerUnit}" /></td>
											...
											<td id="saletabtaxPercentage" class="numeric"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.taxPercentage}" /></td>
											<td class="numeric" id="saletabdiscperc"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.discPer}" /></td>
											<td class="numeric hide" id="sndsaletabdiscperc"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.discPer}" /></td>
											<td class="numeric" id="saletabtotamt"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.recvdItemNetAmount}" /></td>
											<td class="numeric hide" id="sndsaletabtotamt"><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${stockTransferDetail.sendItemNetAmount}" /></td>
											<c:if test="${stockTransferHeader.dispatchStatus =='N'}">
											<td>
													<%-- <c:if test="${saleHeaderDTO.holdFlag ==0}"> --%>
														<button class="btn btn-theme04 btn-xs" title='Delete item' id="${stockTransferDetail.itemUniqueKey}" onclick='javascript:showSelTabItemDelModal(this.id);'>
											...
											<td id="saletabdisc" class="hide">${stockTransferDetail.recvdItemDiscAmount}</td>
											<%-- <td id="saletabcontent" class="hide">${stockTransferDetail.contentName}</td> --%>
											<td id="saletabitemstkqty" class="hide">${stockTransferDetail.recvdPackQty}</td>
											<td id="saletabmrpperunit" class="hide">${((stockTransferDetail.recvdPackQty*stockTransferDetail.conversion)+stockTransferDetail.recvdLooseQty)*stockTransferDetail.mrpPerUnit}</td>
											<td id="saletabrate" class="hide">${stockTransferDetail.rate}</td>
											<%-- <td id="saletabschename" class="hide">${stockTransferDetail.scheduleName}</td> --%>
											<td id="saletabtaxperc" class="hide">${stockTransferDetail.taxPercentage}</td>
											<td id="saletabtax" class="hide">${stockTransferDetail.recvdItemTaxAmount}</td>
											<%-- <td id="saletabcontentid" class="hide">${saleDetail.contentId}</td> --%>
											<td id="saletabtaxId" class="hide">${stockTransferDetail.taxId}</td>
											<td class="numeric hide" id="salerettablqty">${stockTransferDetail.actSendLooseQty}</td>

											<%-- <td id="saletabisGroupTax" class="hide">${stockTransferDetail.isGroupTax}</td> --%>
											<td id="saletabdiscount" class="hide">${stockTransferDetail.recvdItemDiscAmount}</td>
											<td id="sndsaletabdiscount" class="hide">${stockTransferDetail.sendItemDiscAmount}</td>
											<%-- <td id="saletabisDiscount" class="hide">${stockTransferDetail.isDiscount}</td> --%>
											<%-- <td id="saletabmaxDiscountLimit" class="hide">${stockTransferDetail.maxDiscountLimit}</td> --%>
											<td id="saletabitemcalcgstamt" class="hide">${stockTransferDetail.recvdItemTaxAmount}</td>
											<td id="sndsaletabitemcalcgstamt" class="hide">${stockTransferDetail.sendItemTaxAmount}</td>
											<%-- <td id="saletabitemtaxmode" class="hide">${stockTransferDetail.taxMode}</td> --%>
											<td id="saletabitemhsncode" class="hide">${stockTransferDetail.hsnCode}</td>
											<td id='saletabpurcost' class='hide'><fmt:formatNumber type="number" maxFractionDigits="4" minFractionDigits="4" groupingUsed="false" value="${((stockTransferDetail.sendPackQty*stockTransferDetail.conversion)+stockTransferDetail.sendLooseQty)*stockTransferDetail.purchaseRate}" /></td>
											...
											<td class="hide"><input type="text" id="stocktransIdshow"  value="${stockTransferHeader.transId}" /></td>
							                <td class="hide"><input type="text" id="stocktransnoshow" value="${stockTransferHeader.stockTransNo}"/>	</td>									
						...
						<table  id="final_table8">
							<tr>
					           ...
					           <td style="padding-right: 5px;"><input class="form-control-trx" id="remarks" type="text" value="${saleHeaderDTO.remarks}"></td> --%>
								<td ><spring:message code="purinvdet.jsp.total" text="Total:" />:</td>
								<td style="padding-right: 5px;"><input class="form-control-trx" style="color: #000000;" type="text" id="totgrossamt" value="${stockTransferHeader.recvdGrossAmount}" tabindex="-1" readonly>
								    <input class="form-control-trx hide"  type="text" id="hiddentotgrossamt" value="${stockTransferHeader.sendGrossAmount}" tabindex="-1" readonly>
								</td>
								<td class="hide"><input type="text" id="sndtotgrossamt" value="${stockTransferHeader.sendGrossAmount}"></td>
								<c:if test="${mrpEnableFlag==1}">
								<td >Tot.MRP:</td>
								<td style="padding-right: 5px;"><input class="form-control-trx" style="color: #000000;" type="text" id="totmrpamt" value="${stockTransferHeader.recvdTotalMrp}" tabindex="-1" readonly></td>
								
								</c:if>
								<td class="hide"><input  type="text" id="sendtotmrpamt" value="${stockTransferHeader.sendTotalMrp}"></td>
								<c:if test="${mrpEnableFlag==0}">
								<td class="hide">Tot.MRP:</td>
								<td class="hide" style="padding-right: 5px;"><input class="form-control-trx" style="color: #000000;" type="text" id="totmrpamt" value="${stockTransferHeader.recvdTotalMrp}" tabindex="-1" readonly></td>
								</c:if>
								...
								<td  style="padding-right: 5px;" class="hide"><input class="form-control-trx" type="text" id="tottaxamt" value="${stockTransferHeader.recvdTaxAmount}" tabindex="-1" readonly></td>
								<!--  for total tax -->
								
								<td width="10%" style="padding-right: 5px;"><input class="form-control-trx" type="text" id="totgstamt" value="${stockTransferHeader.recvdTaxAmount}" tabindex="-1" readonly></td>
								<td width="10%" style="padding-right: 5px;" class="hide"><input class="form-control-trx" type="text" id="sndtotgstamt" value="${stockTransferHeader.sendTaxAmount}" tabindex="-1" readonly></td>
								<!-- for discount  -->
								<td  width="7%" ><spring:message code="purinvdet.jsp.totdisc" text="Tot.Disc:" /></td>
								<td  width="7%" style="padding-right: 5px;"><input class="form-control-trx" type="text" id="totdiscamt" value="${stockTransferHeader.recvdDiscAmount}" tabindex="-1" readonly></td>
								<td  width="7%" style="padding-right: 5px;"><input class="form-control-trx hide" type="text" id="sndtotdiscamt" value="${stockTransferHeader.sendDiscAmount}" tabindex="-1"></td>
								...
								<td width="7%" style="padding-right: 5px;"><input class="form-control-trx" type="text" id="roundoff" value="${stockTransferHeader.recvdRoundoff}" tabindex="-1" readonly="readonly"></td>
								<td class="hide"><input type="text" id="sndroundoff" value="${stockTransferHeader.sendRoundoff}"></td>
								   <!-- for other adjasment -->
							   ...
							   <%-- <td  class="hide"  width="7%" style="padding: 2px 5px 0 0" ><input  onkeydown="numcheck(event)" class="form-control-trx" type="text" id="otheradjasment" value="${saleHeaderDTO.othAdjAmount}" tabindex="-1" > --%>
								...
								<td width="15%"  ><input class="form-control-trx" style="font-weight: bold; color: #000000; background-color: #ebccd1;" type="text" id="nettot" value="${stockTransferHeader.recvdNetAmount}" tabindex="-1" readonly></td>
							<td class="hide"><input  type="text" id="sndnettot" value="${stockTransferHeader.recvdNetAmount}"></td>
							</tr>
							...
							<tr>
								...
								<td colspan="3" style="padding: 3px 5px 0 0"><input class="form-control-trx" id="remarks" type="text" value="${stockTransferHeader.receiveRemarks}"></td>															
									<c:if test="${stockTransferHeader.recvIsPosted==0}">										
									<td colspan="3" style="padding: 3px 5px 0 0">
									<c:if test="${stockTransferHeader.receiveStatus=='Y'}">																		
											<button type="button" class="btn btn-warning btn-sm" style="padding: 5px 30px;" onclick="closeStockTransferReceive(${stockTransferHeader.transId})">
											<spring:message code="pos.jsp.closestock" text="Close"  />											
											</button>	
											<button class="btn btn-primary btn-sm" style="padding: 5px 30px;" title="Save Button " type="button" id="saveStockButtId" onclick="createOrUpdateStockTransfer()">
												<spring:message code="pos.jsp.stocktransferSave.btn" text="SAVE" />
											</button>																																
											</c:if>	
			...
			<div id="toppanel2" class="panel-trx panel-default">
				...
				<div class="panel-body-trx">
					<table>
                        <td class="hide"><input  type="text" style="padding: 4px 1px;" id="retmemoFinyr" value="${sessionScope.sesloggedinUser.finyrCode}" size="5"></td>
					</table>
				</div>
			</div>
		</div>
...
<div class="modal fade" id="confirmModalStock" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<!-- <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button> -->
			...
			<div class="modal-footer">
				...
				<button type="button" onclick="DoConfirmCloseStkRec()" data-dismiss="modal" class="btn btn-theme">
					<spring:message code="footer.jsp.btn.ok" text="OK" />
				</button>
			</div>
		</div>
...
<script src="${pageContext.request.contextPath }/assets/js/inventory/stock/stock_receive.js"></script>
<script src="${pageContext.request.contextPath }/assets/js/glitter/jquery.gritter.js"></script>
<c:if test="${pageContext.response.locale == 'en'}">
	<script src="${pageContext.request.contextPath }/assets/js/common/common_en.js"></script>
</c:if>
<script src="${pageContext.request.contextPath }/assets/js/inventory/stock/stock_trans_en.js"></script>
