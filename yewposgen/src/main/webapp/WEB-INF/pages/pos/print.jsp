			...
			<div style="float: center; height: auto; width: 100%; margin-bottom: 5px;">
				<div id="billinginfo_div">
   			    ...
   			    <c:if test="${saleHeaderDTO.esiType!=''}">
					...
					<div>
						<label style="width: 10%;"><spring:message code="cmn.print.jsp.trn" text="GST" />.:</label> <label id="patientName_label">${saleHeaderDTO.gstNo}</label>

					</div>
					<div>
						<label style="width: 10%;"><spring:message code="cmn.print.jsp.dlno" text="DL No." />.:</label> <label id="patientName_label">${saleHeaderDTO.dlNo}</label>

					</div>
					<div>
						<label style="width: 10%;"><spring:message code="cmn.print.jsp.panno" text="PAN No." />.:</label> <label id="patientName_label">${saleHeaderDTO.panNo}</label>
				...
				<table id="billitemtable" style="text-align: center; border-collapse: collapse; width: 100%;">
					<thead style="border-bottom: 1px dashed;">
						<tr>
							...
							<th><spring:message code="item.jsp.name" text="Item Name" /><br>
...
<%-- 							<th><spring:message code="cmn.print.jsp.hsn" text="HSN" /></th> --%>
						...
						<th width="5%;"><spring:message code="purinvdet.jsp.lqty" text="L.Qty" /></th> --%>
							...
							<!--  for amount  -->
							<th>&nbsp;<spring:message code="reprintcash.jsp.taxableamt" text="Taxable Amt." />&nbsp;<br>
					...
					<tbody style="border-bottom: 1px dashed;">
						...
						<c:if test="${!empty saleDetailsDTOs }">
							...
							<c:forEach items="${saleDetailsDTOs}" var="saleDetail" varStatus="loop">
								<tr>
									<td>${loop.index+1}</td>
									<td style="text-align: left; white-space: nowrap;">${saleDetail.itemName}SL#${saleDetail.serialNoDtl}</td>
...
<%-- 									<td>${saleDetail.hsnCode}</td> --%>
									...
									<td> ${saleDetail.itemdualstock==1 ?(saleDetail.packQty):(saleDetail.packQty * saleDetail.conversion + saleDetail.looseQty)}</td>
									...
									<td width="5%;" style="text-align: right;"><fmt:formatNumber type="number" minFractionDigits="2" maxFractionDigits="2" value="${saleDetail.ratePerUnit}" /></td>
			...
			<div style="margin-bottom: 1px;">
				<table style="width: 100%;">
					...
					<tr>
						<td><spring:message code="reprintcash.jsp.totinvword" text="Total Invoice Value (In Words)" />: <span id="totInvValueWord" style="text-align: left; white-space: nowrap; font-weight: normal;"> </span></td>
						<!-- <td id="totInvValueWord" style="text-align: left;white-space:nowrap;"></td> -->
					</tr>
					<tr id="reversetaxRow">
						<td><spring:message code="reprintcash.jsp.taxamtrspctrvrschrg" text="Amt of Tax subject to Rev. Chrgs" />: <span style="font-weight: bold;"> CGST = <fmt:formatNumber value="${saleHeaderDTO.cgst}" currencySymbol="" type="currency" /> SGST = <fmt:formatNumber value="${saleHeaderDTO.sgst}" currencySymbol="" type="currency" />
						</span></td>
